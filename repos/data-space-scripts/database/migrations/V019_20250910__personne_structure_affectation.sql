-- Déplacement des affectations de structure vers la table personne_affectations
INSERT INTO main.personne_affectations (personne_id, structure_id, type)
SELECT id, structure_id, 'structure_emploi'
FROM main.personne
WHERE structure_id IS NOT NULL
AND NOT EXISTS (SELECT 1 FROM main.personne_affectations aff WHERE aff.personne_id = personne.id AND aff.structure_id = personne.structure_id AND type = 'structure_emploi');

-- Suppression des vues dépendantes de la colonne structure_id
DROP VIEW dataviz.lieux_inclusion_numerique;
DROP VIEW dataviz.structures_employeuses;

-- Suppression de la colonne structure_id
ALTER TABLE main.personne DROP COLUMN structure_id;


-- Recréation des vues modifiées
...
ALTER TABLE main.activites_coop ADD COLUMN periode date GENERATED ALWAYS AS (date_trunc('month', date::timestamp without time zone)::date) STORED;
CREATE INDEX  activites_coop_periode_idx ON main.activites_coop (periode);
...
CREATE VIEW dataviz.personnes_accompagnements AS (
   WITH src AS (
    ...
    counts AS (  -- table “longue” agrégée une fois
      -- TYPE
      SELECT personne_id, periode, 'type'::text AS dim, type::text AS key, COUNT(*)::bigint AS n
      FROM src
      ...
      UNION ALL
      -- AUTONOMIE
      SELECT personne_id, periode, 'autonomie', autonomie::text, COUNT(*)::bigint
      FROM src
      ...
      UNION ALL
      -- TYPE_LIEU
      SELECT personne_id, periode, 'type_lieu', type_lieu::text, COUNT(*)::bigint
      FROM src
      ...
      UNION ALL
      -- THEMATIQUES
      SELECT s.personne_id, s.periode, 'thematique', t::text, COUNT(*)::bigint
      FROM src s
    ...
    FROM base b
    LEFT JOIN counts c
      ON c.personne_id = b.personne_id
     AND c.periode     = b.periode
    GROUP BY
      b.periode, b.personne_id, b.nom, b.prenom, b.role, b.nb_accompagnements
    ORDER BY
      b.periode, b.role, b.personne_id
);
...
CREATE OR REPLACE VIEW dataviz.accompagnements AS (
    SELECT periode,
        t AS thematique,
        COUNT(*) AS nb_accompagnements
    FROM main.activites_coop AS a
    LEFT JOIN LATERAL unnest(COALESCE(a.thematiques, ARRAY[]::text[])) AS t ON true
    GROUP BY periode, t
    ORDER BY periode
);

COMMENT ON COLUMN dataviz.accompagnements.periode IS 'Mois des accompagnements (format YYYY-MM-DD), ramené au premier jour du mois.';
COMMENT ON COLUMN dataviz.accompagnements.nb_accompagnements IS 'Nombre d''accompagnements réalisés dans le mois sur la thématique.';

CREATE UNIQUE INDEX personne_affectations_unique_key ON main.personne_affectations (structure_id, personne_id, type, COALESCE(suppression, '1234-01-02 03:04:05'));
...
CREATE VIEW dataviz.personne AS (
    ...
    FROM main.personne
    LEFT JOIN main.personne_affectations ON personne.id = personne_affectations.personne_id AND type = 'structure_emploi' AND personne_affectations.suppression IS NULL AND personne_affectations.structure_id IS NOT NULL
