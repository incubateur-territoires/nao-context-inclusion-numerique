-- Migration V054: Fix vue min.postes_conseiller_numerique_synthese
-- Corrige le mélange de données entre personnes différentes pour un même poste.
--
-- Problème : le CTE poste_par_structure sélectionnait la personne la plus récente
-- (ORDER BY created_at DESC), mais les CTEs contrats_en_cours_par_poste et
-- dernier_contrat travaillaient sur main.poste complet (toutes les personnes
-- historiques). Résultat : personne affichée, date_fin_contrat et nb_contrats
-- pouvaient provenir de personnes différentes.
--
-- Corrections :
-- 1. poste_par_structure : prioriser la personne ayant un contrat actif
-- 2. contrats_en_cours_par_poste et dernier_contrat : référencer poste_par_structure
--    au lieu de main.poste pour assurer la cohérence avec la personne sélectionnée
