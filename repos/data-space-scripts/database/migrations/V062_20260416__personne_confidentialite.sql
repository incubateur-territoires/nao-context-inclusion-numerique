-- V062 – Confidentialité des personnes (is_visible)
--
-- Ajoute une colonne main.personne.is_visible (NULL par défaut = visible)
-- et EXCLUT entièrement les personnes dont is_visible = FALSE de :
--   - la liste des médiateurs renvoyée par api.carto
--   - la recherche api.get_carto_mediateur

ALTER TABLE main.personne ADD COLUMN is_visible BOOLEAN DEFAULT NULL;
...
-- Redéfinition de api.carto : identique à V061 (filtre lieux actifs),
-- avec exclusion des personnes dont is_visible = FALSE dans l'agrégat
-- mediateurs (les deux branches CN et AC du UNION).
...
-- Redéfinition de api.get_carto_mediateur : base V029 + filtre is_visible.
-- IS DISTINCT FROM FALSE inclut is_visible IS NULL et is_visible = TRUE.
