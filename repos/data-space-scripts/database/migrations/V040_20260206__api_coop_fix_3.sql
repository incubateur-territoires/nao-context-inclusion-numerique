...
BEGIN
    ...
    contrat_actif AS (
        SELECT contrat.personne_id
        FROM main.contrat
        WHERE contrat.personne_id = var_personne_id
        AND contrat.date_rupture IS NULL
        LIMIT 1
    )
    SELECT jsonb_build_object(
        'id', personne.id,
        'is_conseiller_numerique', CASE WHEN contrat_actif.personne_id IS NOT NULL THEN True ELSE False END,
        'pg_id', personne.cn_pg_id,
    ...
    LEFT JOIN contrat_actif ON contrat_actif.personne_id = personne.id
    WHERE personne.id = var_personne_id
    GROUP BY personne.id, conseiller_numerique_id, is_coordinateur, structures.structures, cn_coordonnes.conseillers_numerique_coordonnes, lieux_activite.lieux, contrat_actif.personne_id;
END;
$$;
